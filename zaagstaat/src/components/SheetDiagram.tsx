...
import { DIAGRAM_TYPE, truncateDiagramLabel } from '../lib/diagramLabels'
  ...
  return (
    ...
    >
      ...
      {myPlacements.map((pl, i) => {
        ...
        const label = truncateDiagramLabel(part?.label ?? '?', r.w - 24, DIAGRAM_TYPE.label)
        const showLabel = label.length > 0 && r.h >= DIAGRAM_TYPE.label + 16
        const showDims = showLabel && r.h >= DIAGRAM_TYPE.label + DIAGRAM_TYPE.dimensions + 28
        const showBadge = r.w >= DIAGRAM_TYPE.badge && r.h >= DIAGRAM_TYPE.badge
        ...
        return (
          <g key={i}>
            ...
            {showLabel && (
              <text
                x={cx} y={showDims ? cy - 15 : cy}
                textAnchor="middle" dominantBaseline="middle"
                fontSize={DIAGRAM_TYPE.label} fontWeight="700" fill="#2A241D"
            ...
            {showDims && (
              <text
                x={cx} y={cy + 24}
                textAnchor="middle" dominantBaseline="middle"
                fontSize={DIAGRAM_TYPE.dimensions} fill="#5A5142"
            ...
            {showBadge && (
              <>
                <circle
                  cx={r.x + DIAGRAM_TYPE.badge / 2 + 4}
                  cy={r.y + DIAGRAM_TYPE.badge / 2 + 4}
                  r={DIAGRAM_TYPE.badge / 2}
                  fill="#26221C"
                />
                <text
                  x={r.x + DIAGRAM_TYPE.badge / 2 + 4}
                  y={r.y + DIAGRAM_TYPE.badge / 2 + 4}
                  textAnchor="middle" dominantBaseline="middle"
                  fontSize={DIAGRAM_TYPE.badgeText} fontWeight="800" fill="white"
                  fontFamily="Hanken Grotesk, system-ui, sans-serif"
                  style={{ pointerEvents: 'none' }}
                >
                  {seq}
